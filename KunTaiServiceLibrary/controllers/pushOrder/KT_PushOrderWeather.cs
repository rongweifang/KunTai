...
{
    [FluorineFx.RemotingService("坤泰数控供热，四区间设备运行百分比自动生成模板")]
    public class KT_PushOrderWeather
    {
        ...
        private string getCalculateRunPercentageXml(ref DataTable dataTableRunPercentage)
        {
            ...
            xml.Append("<DATAS>");
            KT_PushOrderWeatherObject KT_PushOrderWeatherObject = null;
            foreach (DataRow row in dataTableRunPercentage.Rows)
            {
                KT_PushOrderWeatherObject = new KT_PushOrderWeatherObject(row);
                xml.AppendFormat("<DATA NUM=\"{0}\" ID=\"{1}\" ADDDATETIME=\"{2}\" TIMEPOINT=\"{3}\" W36=\"{4}\" W401=\"{5}\" REVISE=\"{6}\" PERCENTAGE=\"{7}\"/>",
                    KT_PushOrderWeatherObject.NUM,
                    KT_PushOrderWeatherObject.ID,
                    KT_PushOrderWeatherObject.ADDDATETIME,
                    KT_PushOrderWeatherObject.TIMEPOINT,
                    KT_PushOrderWeatherObject.W36,
                    KT_PushOrderWeatherObject.W401,
                    KT_PushOrderWeatherObject.REVISE,
                    KT_PushOrderWeatherObject.PERCENTAGE
                );

                KT_PushOrderWeatherObject = null;
            }
            xml.Append("</DATAS>");
        ...
        private void createRunPercentageDataTable(ref XElement xml, ref DataTable dataTable)
        {
            ...
            foreach (XElement item in xml.Element("DATAS").Elements("DATA"))
            {
                ...
                switch (item.FirstAttribute.Name.ToString())
                {
                    case "H00":
                        ...
                        dataRowNew["W401"] = w707(dataRowNew["W36"].ToString());
            ...
            double w707Sum = Convert.ToDouble(dataTable.Compute("SUM(W401)", string.Empty));
            double reviseSum = this.reviseSum(ref w707Sum);

            w707Sum = Math.Round(w707Sum, 1, MidpointRounding.AwayFromZero);
            reviseSum = Math.Round(reviseSum, 2, MidpointRounding.AwayFromZero);

            //计算每组几个数字和
            int areaCount = dataTable.Rows.Count / 4;
            int index = areaCount;
            double groupSum = 0d;
        ...
        private double reviseSum(ref double w707Sum)
        {
            //=401/354.9*D27
            //
            return 707 * w707Sum / 634.46d ;
        }

        private double w707(string w55)
        {
            //401W=(18/28-C3/28)*36

            //=55*(18/28-C3/28)
            return (18- Convert.ToDouble(w55)) * 55d / 28d;
        }


        private double revise(string w707)
        {
            //=401/354.9*D3
            //=707/634.46*D5
            return 707 / 634.46d * Convert.ToDouble(w707);
        }
